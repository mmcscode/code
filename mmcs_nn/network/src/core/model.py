...
class Model(object):
    ...
    def __init__(self, config, train_set=None):
        self.config = config
        if config['model_name'] in ['MMCS']:
            self.net_module = MMCS
        else:
        ...
        print('num_edge_types: {}'.format(config['num_edge_types']))
    ...
    def predict(self, batch, step, update=True, mode='train'):
        ...
        if mode == 'train':
            ...
            loss = loss / self.config['grad_accumulated_steps']  # Normalize loss
            loss.backward()
...
def cal_code_features(network, ex):
    ...
    # weighted_code = network.global_code_att(code_sequence_embedded, code_sequence_embedded)
...
def cal_query_features(network, ex):
    ...
    # weighted_doc = network.global_sequence_att(doc_words_embedded, doc_words_embedded)
