...
class GraphNN(nn.Module):
    def __init__(self, config):
        ...
        self.doc_graph_type = config['doc_graph_type']
        ...
        if self.doc_graph_type == 'gat':
            self.static_graph_mp = GraphMessagePassing(config)
            self.gat = GraphAttentionLayer(hidden_size, hidden_size, 0.6, 0.2)
            self.static_gru_step = GS(hidden_size, hidden_size)

            self.static_gated_fusion = GF(hidden_size)
            self.graph_update = self.static_graph_update
        elif self.doc_graph_type == 'gcn':
            self.gcn = GCN(config)
            self.graph_update = self.graph_gcn_update
    ...
    def static_graph_update(self, node_feature, edge_vec, adj):
        ...
        for _ in range(self.graph_hops):
            ...
            if self.graph_direction == 'all':
                agg_state = self.gf(fw_agg_state, bw_agg_state)
                node_feature = self.gs(node_feature, agg_state)
            elif self.graph_direction == 'forward':
                node_feature = self.gs(node_feature, fw_agg_state)
            else:
                node_feature = self.gs(node_feature, bw_agg_state)
        return node_feature

    # def graph_attention_update(self, node_state, edge_vec, adj):
    #
    #
    #     # for _ in range(self.graph_hops):
    #     for _ in range(self.graph_hops):
    ...
    #         node_state_list = []
    #         for index in range(node_state.size(0)):
    #             cur_node_weight = node_weight[index, :, :]
    #             cur_node_state = node_state[index, :, :]
    #             new_node_state = torch.mul(cur_node_weight, cur_node_state)
    #             node_state_list.append(new_node_state)
    #
    #         node_state = torch.stack(node_state_list)
    #         # bw_agg_state = self.static_graph_mp.mp_func(node_state, edge_vec, node2edge,
    #         #                                             edge2node)  # (num_nodes, dim)
    #         # fw_agg_state = self.static_graph_mp.mp_func(node_state, edge_vec, edge2node.transpose(1, 2),
    #         #                                             node2edge.transpose(1, 2))
    #         # if self.graph_direction == 'all':
